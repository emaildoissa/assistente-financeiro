...
const colors: Record<string, string> = {
  ...
  pending: 'bg-amber-50 text-warning ring-1 ring-inset ring-warning/20',
  ...
  in_progress: 'bg-primary/10 text-primary ring-1 ring-inset ring-primary/20',
  ...
  income_expense: 'bg-primary/10 text-primary ring-1 ring-inset ring-primary/20',
};

export function Badge({ variant, children }: { variant: string; children: ReactNode }) {
  return (
    <span className={cn('inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium', colors[variant] || 'bg-gray-100 text-text-muted')}>
      {children}
    </span>
  );
}
